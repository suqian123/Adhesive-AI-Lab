...
- “外部计算任务与结果回写”默认只显示运行中、待回写、已回写和失败统计；单任务手工提交收在高级入口中。单任务引擎下拉框只显示当前可执行的项目配置，并自动带入对应命令和结果文件。页面打开时每 5 秒检测一次单任务，完成后自动解析、回写并重训；页面关闭期间完成的任务会在下次打开时自动补回写。
- 正式 AI 筛选和模型归档要求至少一个候选已经完成外部计算并自动回写；只有代理数据或实验记录时可以先保存数据，但不会生成或归档 `proxy-v1` 模型。
